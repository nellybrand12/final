...
export async function seedDatabase() {
  ...
  try {
    // Seed Admin User
    const existingAdmin = await dbInstance.select().from(schema.adminUsers).where(eq(schema.adminUsers.username, 'Leblanc'));
    ...
    // Seed Site Settings
    const existingSettings = await dbInstance.select().from(schema.siteSettings);
