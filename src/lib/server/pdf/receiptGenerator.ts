...
export function formatPriceFCFA(amount: string | number | undefined | null): string {
  if (!amount) return '0';
...
export async function generateBookingReceiptPdf(data: ReceiptData): Promise<Uint8Array> {
  ...
  const colorGold = rgb(0.77, 0.66, 0.50);
  ...
  y -= 14;
  page.drawText(cleanPdfText('Prestige & Raffinement Residentiel - Yaounde, Cameroun'), {
    x: 45,
    y,
    size: 9,
    font: fontRegular,
  ...
  y -= 12;
  page.drawText(cleanPdfText('Quartier Bastos - Tel : +237 699 00 00 00 - reservations@madadjeu-hotel.com'), {
    x: 45,
    y,
    size: 8,
    font: fontRegular,
  ...
  const statusText = booking.status === 'confirmed' ? 'STATUT : CONFIRME' : 'STATUT : EN ATTENTE';
  ...
  page.drawText(cleanPdfText('Ref. Reservation :'), { x: 55, y: clientY, size: 8, font: fontRegular, color: colorMuted });
  page.drawText(cleanPdfText(booking.bookingReference), { x: 135, y: clientY, size: 8, font: fontBold, color: colorCharcoal });
  ...
  page.drawText(cleanPdfText(booking.guestPhone || 'Non renseigne'), { x: 135, y: clientY, size: 8, font: fontRegular, color: colorCharcoal });
  ...
  page.drawText(cleanPdfText('DESIGNATION DES PRESTATIONS'), { x: tableX + 12, y: y - 16, size: 8, font: fontBold, color: colorWhite });
  page.drawText(cleanPdfText('QTE'), { x: tableX + 270, y: y - 16, size: 8, font: fontBold, color: colorWhite });
  page.drawText(cleanPdfText('PRIX UNITAIRE'), { x: tableX + 325, y: y - 16, size: 8, font: fontBold, color: colorWhite });
  page.drawText(cleanPdfText('TOTAL (FCFA)'), { x: tableX + 430, y: y - 16, size: 8, font: fontBold, color: colorWhite });
  ...
  const unitRate = room?.pricePerNight ? parseFloat(room.pricePerNight) : (parseFloat(booking.totalPrice) / (nights * roomsCount));
  page.drawText(cleanPdfText(`${room?.name || 'Hebergement'} - ${nights} nuit(s) x ${roomsCount} ch.`), {
    x: tableX + 12,
    y: y - 16,
    size: 8,
    font: fontRegular,
  ...
  page.drawText(cleanPdfText(`${nights * roomsCount}`), { x: tableX + 275, y: y - 16, size: 8, font: fontRegular, color: colorCharcoal });
  page.drawText(cleanPdfText(`${formatPriceFCFA(unitRate)} FCFA`), { x: tableX + 325, y: y - 16, size: 8, font: fontRegular, color: colorCharcoal });
  page.drawText(cleanPdfText(`${formatPriceFCFA(booking.totalPrice)} FCFA`), { x: tableX + 430, y: y - 16, size: 8, font: fontBold, color: colorCharcoal });
  ...
  page.drawText(cleanPdfText('Taxes de sejour, TVA (19.25%) et Services conciergerie inclus'), {
    x: tableX + 12,
    y: y - 16,
    size: 8,
    font: fontItalic,
  ...
  page.drawText('1', { x: tableX + 275, y: y - 16, size: 8, font: fontRegular, color: colorMuted });
  page.drawText('Inclus', { x: tableX + 325, y: y - 16, size: 8, font: fontRegular, color: colorMuted });
  page.drawText('0 FCFA', { x: tableX + 430, y: y - 16, size: 8, font: fontRegular, color: colorMuted });
  ...
  if (booking.paymentTransactionId) {
    ...
    page.drawText(cleanPdfText("Reglement complet a la reception lors du check-in en especes ou carte."), {
      x: 180,
      y: y - 34,
      size: 8,
      font: fontItalic,
  ...
  page.drawText(cleanPdfText('INFORMATIONS IMPORTANTES & CONDITIONS DU SEJOUR'), {
    x: 45,
    y,
    size: 8.5,
    font: fontBold,
  ...
  y -= 14;
  page.drawText(cleanPdfText("- Heure d'arrivee (Check-in) : a partir de 14h00. Heure de depart (Check-out) : jusqu'a 12h00."), {
    x: 45,
    y,
    size: 7.5,
    font: fontRegular,
  ...
  y -= 11;
  page.drawText(cleanPdfText("- Piece d'identite requise : Veuillez presenter une piece d'identite valide et ce recu lors du check-in."), {
    x: 45,
    y,
    size: 7.5,
    font: fontRegular,
  ...
  y -= 11;
  page.drawText(cleanPdfText("- Service Conciergerie & Navette aeroport : Disponible 24h/24 sur simple demande aupres de la reception."), {
    x: 45,
    y,
    size: 7.5,
    font: fontRegular,
  ...
  y -= 16;
  page.drawText(cleanPdfText("Nous vous remercions de votre confiance et vous souhaitons un sejour inoubliable a l'Hotel Residence Madadjeu."), {
    x: 45,
    y,
    size: 8,
    font: fontItalic,
  ...
  y -= 12;
  page.drawText(cleanPdfText('Document genere electroniquement par le systeme de reservation Madadjeu - Fait foi de confirmation officielle.'), {
    x: 45,
    y,
    size: 7,
    font: fontRegular,
