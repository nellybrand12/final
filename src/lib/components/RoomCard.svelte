  ...
  <div class="relative aspect-[16/10] w-full overflow-hidden bg-deep-charcoal">
    ...
    <div class="absolute bottom-3.5 left-3.5 flex items-center text-soft-cream font-label-caps text-[11px] sm:text-xs bg-neutral-950/80 backdrop-blur-sm px-3 py-1.5 border border-white/15 shadow-md">
      <span class="flex items-center gap-1.5">
        <span class="material-symbols-outlined text-sm text-muted-gold">{room.type === 'hall' ? 'celebration' : 'group'}</span>
        <span>
          {#if room.type === 'hall'}
  ...
  <div class="p-5 sm:p-6 lg:p-7 flex flex-col flex-grow justify-between gap-4 w-full">
    <div>
      <div class="flex items-center justify-between gap-2 mb-1">
        <span class="text-[10px] font-label-caps text-muted-gold uppercase tracking-wider font-semibold">
          {room.category}
        </span>
        {#if room.type === 'hall'}
          <span class="text-[9px] font-label-caps px-2 py-0.5 bg-muted-gold/15 text-muted-gold border border-muted-gold/30 rounded-full font-bold">
