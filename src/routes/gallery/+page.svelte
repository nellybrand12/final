  ...
  <div class="max-w-[1280px] mx-auto px-4 md:px-8 lg:px-16">
    ...
    <div class="flex flex-wrap items-center justify-center gap-2 mb-12">
      <button
        ...
        class="px-4 py-1.5 font-label-caps text-xs transition-all {activeFilter === 'all' ? 'bg-deep-charcoal text-soft-cream shadow-md font-bold' : 'bg-surface-container text-on-surface hover:bg-surface-variant'}"
      >
        {i18n.t.gallery.allPhotos} ({galleryItems.length})
      </button>
      <button
        onclick={() => (activeFilter = 'suites')}
        class="px-4 py-1.5 font-label-caps text-xs transition-all {activeFilter === 'suites' ? 'bg-deep-charcoal text-soft-cream shadow-md font-bold' : 'bg-surface-container text-on-surface hover:bg-surface-variant'}"
      >
        {i18n.t.gallery.suitesCat}
      </button>
      <button
        onclick={() => (activeFilter = 'architecture')}
        class="px-4 py-1.5 font-label-caps text-xs transition-all {activeFilter === 'architecture' ? 'bg-deep-charcoal text-soft-cream shadow-md font-bold' : 'bg-surface-container text-on-surface hover:bg-surface-variant'}"
      >
        {i18n.t.gallery.architectureCat}
      </button>
      <button
        onclick={() => (activeFilter = 'excursions')}
        class="px-4 py-1.5 font-label-caps text-xs transition-all {activeFilter === 'excursions' ? 'bg-deep-charcoal text-soft-cream shadow-md font-bold' : 'bg-surface-container text-on-surface hover:bg-surface-variant'}"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each filteredItems as item, index}
        <button
          ...
          class="group relative aspect-[4/3] overflow-hidden bg-deep-charcoal text-left focus:outline-none cursor-pointer shadow-sm hover:shadow-xl transition-all duration-500 border border-outline-variant/30"
        >
          <img
            src={item.imageUrl}
            alt={item.title}
